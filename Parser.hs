...
happyExpList = Happy_Data_Array.listArray (0,24) ([128,2048,0,0,16384,0,8192,0,4,128,8192,0,1,256,2048,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_happyParser","Program","FunctionDecl","Statement","Expr","'int'","'main'","'('","')'","'{'","'}'","'return'","'2'","';'","digits","alphas","ident","%eof"]
        bit_start = st * 20
        bit_end = (st + 1) * 20
...
happyReduction_2 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn5
		 (ASTFunctionDef happy_var_2 (ASTStatement happy_var_6)
	) `HappyStk` happyRest
...
happyReduction_3 _
	...
	_
	 =  HappyAbsSyn6
		 (ASTStatement happy_var_2
	)
happyReduction_3 _ _ _  = notHappyAtAll 

happyReduce_4 = happySpecReduce_1  7 happyReduction_4
happyReduction_4 (HappyTerminal (TokenLit happy_var_1))
	 =  HappyAbsSyn7
		 (ASTExpr happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	Token2 -> cont 15;
	TokenSemic -> cont 16;
	TokenLit happy_dollar_dollar -> cont 17;
